...
main()
{
    ...
    while (1)
    {
        ...
        send(fd2, buf, 100, 0); // send to client 2

        for (i = 0; i < 100; i++)
            buf[i] = '\0';
        size = recv(fd2, buf, 100, 0); // receive from client 2

        printf("\nMessage received from client %s:%d: %s\nNumber of bits: %z\n", inet_ntoa(ta2.sin_addr), ntohs(ta2.sin_port), buf, size);
        send(fd1, buf, 100, 0); // send to client 1
        printf("Message sent backto client\n", buf);
    }
    close(fd1);
}
